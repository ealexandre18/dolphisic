...
# Replacements (using unicode escapes for safe ASCII-only representation)
...
# Programmatic cleanup of bracket order
rep_bip_tbody = rep_bip_tbody.replace("])})", "]})")
rep_active_tbody = rep_active_tbody.replace("])})", "]})")
rep_pending_tbody = rep_pending_tbody.replace("])})", "]})")
