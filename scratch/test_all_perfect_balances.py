...
check_balance("Rep 1", replacement1_base + "]})})]},e.id))")
check_balance("Rep 2", replacement2_base + "])})]},e.id))")  # wait, replacement2 has `(0,P.jsxs)(td, {style:F.actionsCell, children: [...]})`
# Let's verify brackets for Replacement 2:
# td has children: [...]. So td starts with (0,P.jsxs)(`td`, {..., children: [
# To close it:
# 1. ] closes children array
# 2. } closes props object
# 3. ) closes td call
# So td closes with ]})!
# tr has children: [..., td]. So tr starts with (0,P.jsxs)(`tr`, {..., children: [
# To close it:
# 1. ] closes children array
# 2. } closes props object
# 3. ,e.id
...
# 5. ) closes map call
# Let's combine:
# After button: (0,P.jsx)(Ne,{size:14})}) (closes Ne and button)
# Then td closes with: ]})
# Then tr closes with: ]},e.id)
# Then map closes with: )
# So it is: ]}) ]},e.id)) -> ]})]},e.id))!
# Yes! `]})]},e.id))` is exactly what we have!
check_balance("Rep 3", replacement3_base + "]})})]},e.id))")
check_balance("Rep 4", replacement4_base + "]})})]},e.id))")
